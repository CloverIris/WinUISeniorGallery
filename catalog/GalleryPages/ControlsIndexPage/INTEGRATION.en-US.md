...
- Input: Feature Catalog kind=control|behavior.
...
- Register `/controls` as `route.gallery.controls` exactly once.
- Accept only `q, category, priority, maturity`.
...
- Event prefix: `gallery.controls`.
- `gallery.controls.load_started`: route and catalog version.
- `gallery.controls.load_completed`: duration and valid/invalid counts.
- `gallery.controls.state_changed`: previous/next state and reason enum.
- `gallery.controls.navigation_requested`: target feature ID and result.
- `gallery.controls.error`: stable code and correlation ID only.
- Events contain no user content and obey host diagnostics policy.
- Counters are window-scoped and disposed with the page.

## Fallbacks

- Empty: provide Clear filters and the current filter summary.
- Error: show Retry, Return home, and correlation ID.
- Permission/capability: read no files or search history.
