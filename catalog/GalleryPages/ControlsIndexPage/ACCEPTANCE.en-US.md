# ControlsIndexPage Acceptance

## Routing and loading

- Given `route.gallery.controls` is registered, when navigating to `/controls`, then one page instance is created in the target window scope.
- Given valid `q, category, priority, maturity`, when loading, then normalized parameters, selection, and anchor are restorable.
...
- Given no matching data, when Empty appears, then provide Clear filters and the current filter summary.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then read no files or search history.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will open the control detail route exactly once.
