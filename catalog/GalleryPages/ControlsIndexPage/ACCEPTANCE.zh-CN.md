...
- Given no matching data, when Empty appears, then 提供清除筛选操作和当前筛选摘要.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then 不读取文件或搜索历史.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will 打开控件详情 route exactly once.
