...
- Given no matching data, when Empty appears, then 目录为空时仍显示产品主张和固定分类入口.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then 不请求权限或网络.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will 打开精选条目的规范 route exactly once.
