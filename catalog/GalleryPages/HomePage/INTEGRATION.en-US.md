...
- Input: Feature Catalog + asset manifest.
...
- Register `/` as `route.gallery.home` exactly once.
- Accept only `section, feature`.
...
- request no permission or network.
...
- Event prefix: `gallery.home`.
- `gallery.home.load_started`: route and catalog version.
- `gallery.home.load_completed`: duration and valid/invalid counts.
- `gallery.home.state_changed`: previous/next state and reason enum.
- `gallery.home.navigation_requested`: target feature ID and result.
- `gallery.home.error`: stable code and correlation ID only.
- Events contain no user content and obey host diagnostics policy.
- Counters are window-scoped and disposed with the page.

## Fallbacks

- Empty: retain the product statement and fixed category entries when the catalog is empty.
- Error: show Retry, Return home, and correlation ID.
- Permission/capability: request no permission or network.
