# WindowingIndexPage Acceptance

## Routing and loading

- Given `route.gallery.windowing` is registered, when navigating to `/windowing`, then one page instance is created in the target window scope.
- Given valid `feature, support`, when loading, then normalized parameters, selection, and anchor are restorable.
...
- Given no matching data, when Empty appears, then retain capability summary without creating another window.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then enumerate no other process windows and request no system permission.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will open detail first and let detail request window operations exactly once.
