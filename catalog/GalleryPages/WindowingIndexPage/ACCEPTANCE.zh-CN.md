...
- Given no matching data, when Empty appears, then 保留能力摘要且不创建额外窗口.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then 不枚举其他进程窗口或请求系统权限.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will 先打开详情，再由详情请求窗口操作 exactly once.
