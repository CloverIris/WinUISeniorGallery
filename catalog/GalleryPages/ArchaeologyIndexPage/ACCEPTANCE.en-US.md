# ArchaeologyIndexPage Acceptance

## Routing and loading

- Given `route.gallery.archaeology` is registered, when navigating to `/archaeology`, then one page instance is created in the target window scope.
- Given valid `period, product, source, license`, when loading, then normalized parameters, selection, and anchor are restorable.
...
- Given no matching data, when Empty appears, then retain the non-affiliation notice, research method, and filter summary.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then download no remote historical asset and request no account.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will open local research; external links require another explicit action exactly once.
