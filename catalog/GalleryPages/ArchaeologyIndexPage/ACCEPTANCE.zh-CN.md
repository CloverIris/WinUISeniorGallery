...
- Given no matching data, when Empty appears, then 保留非官方声明、研究方法和筛选摘要.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then 不下载远程历史素材或请求账户.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will 打开本地研究页，外链需再次显式激活 exactly once.
