...
- Input: Media catalog + related experiences.
...
- Register `/media` as `route.gallery.media` exactly once.
- Accept only `feature, capability`.
...
- request no media-library, microphone, camera, or network access.
...
- Event prefix: `gallery.media`.
- `gallery.media.load_started`: route and catalog version.
- `gallery.media.load_completed`: duration and valid/invalid counts.
- `gallery.media.state_changed`: previous/next state and reason enum.
- `gallery.media.navigation_requested`: target feature ID and result.
- `gallery.media.error`: stable code and correlation ID only.
- Events contain no user content and obey host diagnostics policy.
- Counters are window-scoped and disposed with the page.

## Fallbacks

- Empty: retain media architecture guidance without creating a player.
- Error: show Retry, Return home, and correlation ID.
- Permission/capability: request no media-library, microphone, camera, or network access.
