...
- Given no matching data, when Empty appears, then 保留媒体架构说明且不创建播放器.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then 不请求媒体库、麦克风、摄像头或网络.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will 打开媒体规格 route exactly once.
