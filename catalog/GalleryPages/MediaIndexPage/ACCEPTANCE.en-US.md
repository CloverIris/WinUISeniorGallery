# MediaIndexPage Acceptance

## Routing and loading

- Given `route.gallery.media` is registered, when navigating to `/media`, then one page instance is created in the target window scope.
- Given valid `feature, capability`, when loading, then normalized parameters, selection, and anchor are restorable.
...
- Given no matching data, when Empty appears, then retain media architecture guidance without creating a player.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then request no media-library, microphone, camera, or network access.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will open the media specification route exactly once.
