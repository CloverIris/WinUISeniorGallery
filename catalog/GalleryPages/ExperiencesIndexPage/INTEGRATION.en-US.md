# ExperiencesIndexPage Integration
...
- Input: Feature Catalog kind=experience + dependency graph.
...
- Register `/experiences` as `route.gallery.experiences` exactly once.
- Accept only `category, input, maturity`.
...
- start no media, multi-window flow, background task, or provider.
...
- Event prefix: `gallery.experiences`.
- `gallery.experiences.load_started`: route and catalog version.
- `gallery.experiences.load_completed`: duration and valid/invalid counts.
- `gallery.experiences.state_changed`: previous/next state and reason enum.
- `gallery.experiences.navigation_requested`: target feature ID and result.
- `gallery.experiences.error`: stable code and correlation ID only.
- Events contain no user content and obey host diagnostics policy.
- Counters are window-scoped and disposed with the page.

## Fallbacks

- Empty: show category guidance and Clear filters.
- Error: show Retry, Return home, and correlation ID.
- Permission/capability: start no media, multi-window flow, background task, or provider.
