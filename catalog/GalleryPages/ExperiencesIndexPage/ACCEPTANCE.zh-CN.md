...
- Given no matching data, when Empty appears, then 显示分类说明和清除筛选.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then 不启动媒体、多窗口、后台任务或 Provider.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will 打开体验规格，依赖完整时才允许运行演示 exactly once.
