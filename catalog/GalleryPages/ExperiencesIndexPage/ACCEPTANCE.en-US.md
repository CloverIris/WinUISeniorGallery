# ExperiencesIndexPage Acceptance

## Routing and loading

- Given `route.gallery.experiences` is registered, when navigating to `/experiences`, then one page instance is created in the target window scope.
- Given valid `category, input, maturity`, when loading, then normalized parameters, selection, and anchor are restorable.
...
- Given no matching data, when Empty appears, then show category guidance and Clear filters.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then start no media, multi-window flow, background task, or provider.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will open the experience specification and enable Run only when dependencies are complete exactly once.
