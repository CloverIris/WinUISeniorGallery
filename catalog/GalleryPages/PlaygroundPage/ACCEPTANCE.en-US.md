# PlaygroundPage Acceptance

## Routing and loading

- Given `route.gallery.playground` is registered, when navigating to `/playground`, then one page instance is created in the target window scope.
- Given valid `demo_id, theme, input`, when loading, then normalized parameters, selection, and anchor are restorable.
...
- Given no matching data, when Empty appears, then show the demo picker and security boundary.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then execute no arbitrary C#/XAML, load no assembly, and write no disk.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will instantiate an allow-listed demo factory and commit typed properties exactly once.
