...
- Given no matching data, when Empty appears, then 显示 demo 选择器与安全边界.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then 不执行任意 C#/XAML、不加载程序集、不写磁盘.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will 实例化白名单 demo 工厂并提交类型化属性 exactly once.
