...
- Input: window-scoped Demo Registry.
...
- Register `/playground` as `route.gallery.playground` exactly once.
- Accept only `demo_id, theme, input`.
...
- execute no arbitrary C#/XAML, load no assembly, and write no disk.
...
- Event prefix: `gallery.playground`.
- `gallery.playground.load_started`: route and catalog version.
- `gallery.playground.load_completed`: duration and valid/invalid counts.
- `gallery.playground.state_changed`: previous/next state and reason enum.
- `gallery.playground.navigation_requested`: target feature ID and result.
- `gallery.playground.error`: stable code and correlation ID only.
- Events contain no user content and obey host diagnostics policy.
- Counters are window-scoped and disposed with the page.

## Fallbacks

- Empty: show the demo picker and security boundary.
- Error: show Retry, Return home, and correlation ID.
- Permission/capability: execute no arbitrary C#/XAML, load no assembly, and write no disk.
