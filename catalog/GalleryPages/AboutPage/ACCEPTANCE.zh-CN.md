...
- Given no matching data, when Empty appears, then 元数据缺失显示 unknown，仍显示 License 与非官方声明.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then 不检查更新或发送遥测.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will 查看本地法律文本、复制白名单诊断或显式打开外链 exactly once.
