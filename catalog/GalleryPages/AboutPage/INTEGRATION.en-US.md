...
- Input: assembly metadata + LICENSE + notice manifest.
...
- Register `/about` as `route.gallery.about` exactly once.
- Accept only `section`.
...
- perform no update check or telemetry.
...
- Event prefix: `gallery.about`.
- `gallery.about.load_started`: route and catalog version.
- `gallery.about.load_completed`: duration and valid/invalid counts.
- `gallery.about.state_changed`: previous/next state and reason enum.
- `gallery.about.navigation_requested`: target feature ID and result.
- `gallery.about.error`: stable code and correlation ID only.
- Events contain no user content and obey host diagnostics policy.
- Counters are window-scoped and disposed with the page.

## Fallbacks

- Empty: show unknown for missing metadata while retaining License and non-affiliation notice.
- Error: show Retry, Return home, and correlation ID.
- Permission/capability: perform no update check or telemetry.
