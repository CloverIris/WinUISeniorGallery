# AboutPage Acceptance

## Routing and loading

- Given `route.gallery.about` is registered, when navigating to `/about`, then one page instance is created in the target window scope.
- Given valid `section`, when loading, then normalized parameters, selection, and anchor are restorable.
...
- Given no matching data, when Empty appears, then show unknown for missing metadata while retaining License and non-affiliation notice.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then perform no update check or telemetry.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will view local legal text, copy allow-listed diagnostics, or explicitly open a link exactly once.
