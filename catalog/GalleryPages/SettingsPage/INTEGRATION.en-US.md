...
- Input: IGallerySettingsStore + validated defaults.
...
- Register `/settings` as `route.gallery.settings` exactly once.
- Accept only `section`.
...
- write local settings only, without roaming, credentials, or network.
...
- Event prefix: `gallery.settings`.
- `gallery.settings.load_started`: route and catalog version.
- `gallery.settings.load_completed`: duration and valid/invalid counts.
- `gallery.settings.state_changed`: previous/next state and reason enum.
- `gallery.settings.navigation_requested`: target feature ID and result.
- `gallery.settings.error`: stable code and correlation ID only.
- Events contain no user content and obey host diagnostics policy.
- Counters are window-scoped and disposed with the page.

## Fallbacks

- Empty: show every default instead of an empty page.
- Error: show Retry, Return home, and correlation ID.
- Permission/capability: write local settings only, without roaming, credentials, or network.
