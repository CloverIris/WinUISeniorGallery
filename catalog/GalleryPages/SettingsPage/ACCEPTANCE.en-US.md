# SettingsPage Acceptance

## Routing and loading

- Given `route.gallery.settings` is registered, when navigating to `/settings`, then one page instance is created in the target window scope.
...
- Given no matching data, when Empty appears, then show every default instead of an empty page.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then write local settings only, without roaming, credentials, or network.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will update window preview and asynchronously persist the validated value exactly once.
