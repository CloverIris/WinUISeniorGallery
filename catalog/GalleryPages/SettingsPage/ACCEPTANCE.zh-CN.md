...
- Given no matching data, when Empty appears, then 显示所有默认值而不是空白页.
- Given provider failure, when Error appears, then Retry and a correlation ID are visible.
- Given permission/capability evaluation, when rendered, then 仅写本地设置，不漫游、不存凭据、不联网.
...
- Given a focused primary action, when Enter/Space/gamepad A is pressed, then it will 更新窗口预览并异步持久化验证值 exactly once.
