...
深度 20 的路径在 320 epx 下折叠但当前项与最近祖先可见；编辑无效路径不改变当前位置，修正后只发一次导航请求。

## 输入、无障碍与性能

F4/Ctrl+L 进入编辑，Enter 提交，Esc 取消；鼠标、触摸、键盘与屏幕阅读器均可展开被折叠层级。 Light、Dark、High Contrast、Reduced Motion、RTL、100%–300% DPI 和 200% 文本缩放通过。关闭页面后无 Provider、事件、计时器或窗口泄漏。
