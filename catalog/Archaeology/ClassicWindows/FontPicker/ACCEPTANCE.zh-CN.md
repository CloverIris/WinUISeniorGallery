...
枚举 5,000 字体面时列表保持虚拟化；缺失/受限字体显示确定性回退；选择字体只返回描述符，不复制或嵌入字体文件。

## 输入、无障碍与性能

搜索支持 IME，方向键浏览，Enter 预览/提交按宿主策略区分；屏幕阅读器读字体名和状态，不尝试描述视觉字形。 Light、Dark、High Contrast、Reduced Motion、RTL、100%–300% DPI 和 200% 文本缩放通过。关闭页面后无 Provider、事件、计时器或窗口泄漏。
