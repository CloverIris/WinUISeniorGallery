...
恢复期间切换选择不会并发恢复两个会话；过期状态明确标注并只提供重新开始，最近排序在刷新后保持稳定。

## 输入、无障碍与性能

横向卡片支持方向键/手柄、鼠标滚轮、触摸和屏幕阅读器；恢复确认不能依赖长按。 Light、Dark、High Contrast、Reduced Motion、RTL、100%–300% DPI 均可完成核心路径。演示不得持续轮询或后台联网，关闭后无计时器、事件或窗口泄漏。
