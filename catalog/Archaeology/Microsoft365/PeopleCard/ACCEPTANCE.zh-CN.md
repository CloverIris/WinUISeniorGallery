...
无网络且只有姓名时仍显示可访问的最小卡片；慢速 section 完成后不抢焦点，关闭卡片会取消加载并忽略迟到结果。

## 输入、无障碍与性能

Hover 仅预览，键盘焦点或点击可固定；Esc 关闭并恢复触发元素；触摸没有 hover 依赖，状态和头像均有文本替代。 Light、Dark、High Contrast、Reduced Motion、RTL、100%–300% DPI 和 200% 文本缩放通过。演示关闭后无事件、请求、计时器或窗口泄漏。
