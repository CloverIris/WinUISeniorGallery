...
定义网络、ASR、翻译和内容 Provider 的抽象、凭据、取消、隐私与离线降级。
...
- `ProviderId`: 供应商无关身份.
- `ProviderRequest`: 不可变请求.
- `ProviderResult`: 成功.
- `CorrelationId`: 无内容诊断关联.
...
- 公共模型不暴露供应商 SDK 类型。
- 凭据由宿主安全提供，不进入仓库、日志、设置或诊断包。
- 调用支持取消、超时、限流、离线和权限拒绝。
- 错误转换为稳定领域 code 并保留 CorrelationId。
- 遥测默认不包含媒体、字幕、语音、联系人或个人内容。
- Gallery 默认使用本地 fake provider。
...
- `IExternalProvider<TRequest,TResult>`: 异步可取消调用.
- `ProviderCapabilitySnapshot`: offline.
- `ProviderStatusChanged`: 可用性与限流状态.
...
网络/解析在后台；结果为不可变数据，由消费窗口 Dispatcher 应用；Provider 不持有 UIElement/XamlRoot。
...
timeout/offline/rate-limit/auth/permission/cancelled 有不同稳定 code；失败不清除最后有效缓存。
...
可新增 capability/error code；改变请求/结果字段含义、凭据边界或隐私默认需要 v2。
...
- fake: MUST have deterministic automated evidence.
- 取消: MUST have deterministic automated evidence.
- 凭据与日志扫描: MUST have deterministic automated evidence.
- 多窗口结果隔离: MUST have deterministic automated evidence.
