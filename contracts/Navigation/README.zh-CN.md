...
定义 Gallery 路由、深链接、参数、返回栈、窗口作用域和导航结果。
- 适用于所有消费本契约的生产控件、体验、Gallery 集成、适配器和测试。
- 本契约不把功能专属公共 API 的所有权转移给契约包。

## 术语

- `RouteId`: 稳定 route provider ID.
- `CanonicalRoute`: 小写规范路径.
- `NavigationEntry`: 可恢复导航状态.
- `NavigationResult`: 成功.
...
- Route 以 / 开头、全局唯一、使用小写 kebab-case。
- 参数可序列化、白名单验证且稳定排序。
- 未知 route 显示可恢复 Not Found，不退出进程。
- 导航完成后键盘/深链进入聚焦 H1。
- Back/Forward 恢复参数、选择和滚动锚点。
- 重复目标请求在完成前合并。
...
- `INavigationService`: 窗口作用域导航服务.
- `RouteDescriptor`: route、参数 schema 与页面工厂.
- `NavigationCompleted`: 一次导航结果事件.
...
每个窗口一个 INavigationService 与返回栈；不得跨窗口复用 Page/UIElement。
...
无效参数回退默认并诊断；页面工厂失败显示错误页；窗口关闭取消待处理导航。
...
可新增 route/可选参数；删除 route、改变参数含义或返回栈语义需要 v2 与重定向。
...
- route 唯一: MUST have deterministic automated evidence.
- 深链与无效参数: MUST have deterministic automated evidence.
- Back: MUST have deterministic automated evidence.
