...
- Contract ID: `contract.navigation.v1`
- Dependencies: contract.localization.v1, contract.accessibility.v1
...
Defines Gallery routes, deep links, parameters, back stack, window scope, and navigation results.
...
- `RouteId`: Stable route-provider ID.
- `CanonicalRoute`: Lowercase canonical path.
- `NavigationEntry`: Restorable navigation state.
- `NavigationResult`: 拒绝.
...
- Routes begin with /, are globally unique, and use lowercase kebab-case.
- Parameters are serializable, allow-list validated, and stably ordered.
- Unknown routes show recoverable Not Found and never terminate the process.
- Keyboard/deep-link arrival focuses H1 after navigation.
- Back/Forward restores parameters, selection, and scroll anchor.
- Identical target requests are coalesced until completion.
...
- `INavigationService`: Window-scoped navigation service.
- `RouteDescriptor`: Route, parameter schema, and page factory.
- `NavigationCompleted`: Single navigation-result event.
...
Each window owns one INavigationService and back stack; Page/UIElement instances are never reused across windows.
...
Invalid parameters fall back with diagnostics; factory failure shows an error page; window close cancels pending navigation.
...
Routes/optional parameters may be added; removal or semantic changes need v2 plus redirects.
...
- Route uniqueness: MUST have deterministic automated evidence.
- Deep links and invalid parameters: MUST have deterministic automated evidence.
- Forward 恢复: MUST have deterministic automated evidence.
- Window isolation: MUST have deterministic automated evidence.
