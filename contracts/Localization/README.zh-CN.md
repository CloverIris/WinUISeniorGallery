...
定义 UI 字符串、规格源语言、格式化、回退与 RTL。
- 适用于所有消费本契约的生产控件、体验、Gallery 集成、适配器和测试。
- 本契约不把功能专属公共 API 的所有权转移给契约包。

## 术语

- `SourceLanguage`: 规范源语言.
- `ResourceKey`: 稳定本地化身份.
- `CultureSnapshot`: 窗口当前区域快照.
- `FallbackChain`: 区域资源回退链.
...
- zh-CN 文档是规范源，en-US 标题层级、ID、API 与 route 同步。
- UI 文本使用资源键，禁止拼接自然语言句子。
- 日期、数字、时长、复数和排序遵循 CultureSnapshot。
- RTL 镜像布局，代码、ID、版本和产品名保持语义方向。
- 缺失区域回退 en-US，再回退稳定 ID。
- 资源占位符名称/类型跨语言一致。
...
- `ILocalizationService`: 窗口作用域字符串与格式化服务.
- `CultureChanged`: 区域变化事件.
- `Senior.Strings.<Area>.<Name>`: 公共字符串资源键.
...
CultureSnapshot 按窗口且不可变；CultureChanged 在 UI Dispatcher 发布并触发可控页面刷新。
...
格式化错误显示安全回退并记录资源键，不记录用户值；缺失资源非致命。
...
可新增键/区域；删除键、改变占位符或既有翻译语义需要迁移和 v2。
...
- zh-CN: MUST have deterministic automated evidence.
- RTL: MUST have deterministic automated evidence.
- 格式化: MUST have deterministic automated evidence.
- 缺失资源回退: MUST have deterministic automated evidence.
