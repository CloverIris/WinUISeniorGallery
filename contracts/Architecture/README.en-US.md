...
- Contract ID: `contract.architecture.v1`
- Dependencies: none
...
Defines solution layering, dependency direction, public ownership, feature work items, and compatibility gates.
...
- `FeatureId`: Stable feature identity.
...
- Core never references Gallery, Archaeology, or concrete adapters.
- Controls references Core only; Media references Core+Controls; Windowing references Core only.
- Archaeology may compose those modules; Gallery is the final composition root.
- Cross-module calls use owning contracts and never expose implementation types.
- The depends_on graph is acyclic and every provides ID has one owner.
- Only status=ready work items may enter production implementation.
...
- `feature.json`: Machine-truth manifest.
- `depends_on`: Stable dependency edge.
- `provides`: Public capability ownership.
...
Architecture metadata is a process-read-only immutable snapshot; each window owns its service scope.
...
Dependency reversal, duplicate IDs, cycles, overlapping owned_paths, or unknown providers fail validation.
...
Adding optional fields is backward compatible; removing/renaming stable IDs, dependency semantics, or ownership requires v2.
...
- Dependency graph: MUST have deterministic automated evidence.
- Unique IDs and routes: MUST have deterministic automated evidence.
- owned_paths isolation: MUST have deterministic automated evidence.
- Package reference direction: MUST have deterministic automated evidence.
