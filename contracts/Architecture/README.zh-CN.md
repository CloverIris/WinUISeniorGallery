...
定义解决方案分层、依赖方向、公共所有权、feature 工作单元和兼容性门槛。
...
- Core 不引用 Gallery、Archaeology 或具体适配器。
- Controls 仅引用 Core；Media 引用 Core+Controls；Windowing 仅引用 Core。
- Archaeology 可组合前述模块；Gallery 是最终组合根。
- 跨模块调用只使用所属 Contract，不传递内部实现类型。
- depends_on 图必须无环且所有 provides 只有一个所有者。
- 只有 status=ready 的工作单元可以进入正式实现。
...
架构元数据是进程只读不可变快照；每个窗口只持有自己的服务作用域。
...
依赖反转、重复 ID、循环、重叠 owned_paths 或未知提供者均使验证失败。
...
新增可选字段向后兼容；删除/重命名稳定 ID、依赖语义或公开所有权需要 v2。
...
- 依赖图: MUST have deterministic automated evidence.
- 唯一 ID 与 route: MUST have deterministic automated evidence.
- owned_paths 隔离: MUST have deterministic automated evidence.
- 包引用方向: MUST have deterministic automated evidence.
