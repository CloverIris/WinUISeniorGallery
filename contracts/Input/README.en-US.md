...
- Contract ID: `contract.input.v1`
- Dependencies: contract.accessibility.v1
...
Unifies actions, focus, and conflict handling across mouse, keyboard, touch, pen, touchpad, and gamepad.
...
- `InputAction`: Device-independent user intent.
- `Preview`: Uncommitted input phase.
- `Commit`: Single committed action.
- `Cancel`: Restore last valid state.
...
- Core operations are keyboard accessible and touch gestures have explicit alternatives.
- Preview, Commit, and Cancel remain distinct; one gesture commits once.
- Unconsumed wheel, arrow, and gamepad input bubbles.
- Windows standard shortcuts are never overridden.
- Focus follows visual/document order and popup dismissal restores invoker focus.
- Minimum target is 40×40 epx; preferred touch target is 44×44 epx.
...
- `IInputActionSource`: Publishes device-independent actions.
- `InputActionRequested`: Commit.
- `InputGestureMap`: Action-to-device bindings.
...
Input events stay on the owning XamlRoot Dispatcher; cross-window messages carry InputAction data, never PointerRoutedEventArgs or UIElement.
...
Device disconnect cancels active Preview; binding conflict preserves system/host priority and records diagnostics.
...
Actions and optional bindings may be added; changing existing action semantics or default shortcuts requires v2.
...
- Six input classes: MUST have deterministic automated evidence.
- Commit: MUST have deterministic automated evidence.
- Focus restoration: MUST have deterministic automated evidence.
- Conflict and bubbling: MUST have deterministic automated evidence.
