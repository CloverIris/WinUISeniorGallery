...
- 核心操作必须键盘可完成，触摸手势必须有显式替代。
- Preview、Commit、Cancel 三阶段不得混淆；一次手势只提交一次。
- 未消费的滚轮、方向键和手柄输入必须冒泡。
- 不得覆盖 Windows 标准快捷键。
- 焦点顺序遵循视觉/文档顺序，弹出层关闭恢复调用者焦点。
- 最小目标 40×40 epx，推荐触摸目标 44×44 epx。
...
- `IInputActionSource`: 发布设备无关动作.
- `InputActionRequested`: Preview.
- `InputGestureMap`: 动作到设备绑定.
...
输入事件在所属 XamlRoot Dispatcher；跨窗口只传 InputAction 数据，不传 PointerRoutedEventArgs 或 UIElement。
...
设备断开取消进行中 Preview；冲突绑定保留系统/宿主优先级并记录诊断。
...
可新增动作和可选绑定；改变既有动作语义或默认快捷键需要 v2。
...
- 六类输入设备: MUST have deterministic automated evidence.
- Preview: MUST have deterministic automated evidence.
- 焦点恢复: MUST have deterministic automated evidence.
- 冲突与冒泡: MUST have deterministic automated evidence.
