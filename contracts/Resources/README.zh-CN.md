...
- 公共键为 Senior.<Area>.<Name>，私有键为 Senior.Internal.<FeatureId>.<Name>。
- 公共键只有一个 provides 所有者。
- 覆盖只通过记录的主题扩展点，禁止依赖字典合并偶然顺序。
- 二进制资产登记来源、许可、哈希、用途和尺寸。
- 未经许可的历史 Windows 素材不得打包。
- 资源解析不得自动联网下载。
...
- `IResourceCatalog`: 只读键.
- `ResourceResolutionFailed`: 缺失.
- `Senior.<Area>.<Name>`: 公共键命名.
...
IResourceCatalog 进程只读；返回的 UI 资源在调用窗口 Dispatcher 实例化，二进制流不跨窗口共享可变状态。
...
缺失资源使用记录的系统/语义占位；重复键、哈希不匹配和 blocked 许可使验证失败。
...
- 唯一资源所有权: MUST have deterministic automated evidence.
- 主题合并顺序: MUST have deterministic automated evidence.
- 资产哈希: MUST have deterministic automated evidence.
- 缺失资源占位: MUST have deterministic automated evidence.
