...
定义 WindowId、窗口服务作用域、AppWindow 能力、标题栏、浮窗和生命周期。
...
- `WindowId`: 稳定窗口身份.
- `WindowScope`: Dispatcher.
- `CapabilitySnapshot`: 平台能力快照.
- `PresentationRequest`: 窗口呈现请求.
...
- 每个窗口独立 Dispatcher、XamlRoot、Navigation、Snackbar 和生命周期 token。
- 跨窗口只传可序列化状态或 SessionId，不传 UIElement。
- 自定义标题栏保留系统按钮、Snap Layout、Alt+Space、DPI 与拖动区域。
- 能力检测可为 unknown，不得猜测 supported。
- 关闭窗口取消排队工作并显式交接共享会话。
- 不支持呈现使用记录的降级，不模拟系统 UI。
...
- `IWindowScope`: WindowId、Dispatcher、XamlRoot 和 Cancellation.
- `IWindowCapabilitySnapshot`: supported.
- `IWindowPresentationService`: 显式窗口呈现结果.
...
所有窗口对象仅在所属 Dispatcher；后台任务只持有 WindowId/不可变数据和生命周期 token。
...
窗口关闭返回 cancelled；能力不支持返回 rejected；创建失败清理部分资源并给出稳定 code。
...
可新增 capability；改变 WindowScope 生命周期、传输边界或标题栏保证需要 v2。
...
- 多窗口隔离: MUST have deterministic automated evidence.
- DPI: MUST have deterministic automated evidence.
- 标题栏: MUST have deterministic automated evidence.
- 关闭竞态: MUST have deterministic automated evidence.
