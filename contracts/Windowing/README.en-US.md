...
- Contract ID: `contract.windowing.v1`
- Dependencies: contract.architecture.v1, contract.input.v1, contract.accessibility.v1
...
Defines WindowId, window service scope, AppWindow capabilities, title bars, floating windows, and lifecycle.
...
- `WindowId`: Stable window identity.
- `WindowScope`: XamlRoot.
- `CapabilitySnapshot`: Platform-capability snapshot.
- `PresentationRequest`: Window-presentation request.
...
- Each window owns Dispatcher, XamlRoot, Navigation, Snackbar, and lifecycle token.
- Cross-window messages carry serializable state or SessionId, never UIElement.
- Custom title bars preserve caption buttons, Snap Layout, Alt+Space, DPI, and drag regions.
- Capability detection may be unknown and never guesses supported.
- Window close cancels queued work and explicitly hands off shared sessions.
- Unsupported presentation uses documented fallback and never imitates system UI.
...
- `IWindowScope`: Window identity and services.
- `IWindowCapabilitySnapshot`: unsupported.
- `IWindowPresentationService`: Explicit presentation result.
...
Window objects stay on their Dispatcher; background work holds WindowId/immutable data and lifecycle token only.
...
Closed windows return cancelled; unsupported capabilities return rejected; creation failure cleans partial resources and returns stable code.
...
Capabilities may be added; changing WindowScope lifecycle, transfer boundaries, or title-bar guarantees requires v2.
...
- 显示器变化: MUST have deterministic automated evidence.
- Snap: MUST have deterministic automated evidence.
- 呈现降级: MUST have deterministic automated evidence.
