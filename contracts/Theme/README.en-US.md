...
Defines Light, Dark, High Contrast, backdrops, semantic tokens, and control-template resources.
...
- `ThemeToken`: Semantic visual value.
- `ThemeVariant`: Light, Dark, or High Contrast.
- `ThemeResource`: Runtime theme resource.
- `FallbackToken`: System fallback resource.
...
- Colors, brushes, corners, shadows, type, and spacing come from semantic resources.
- Control templates never hard-code theme colors or use opacity as the sole state.
- A missing Senior token falls back to a documented WinUI system resource.
- High Contrast removes material dependency and preserves visible boundaries.
- Theme changes do not recreate pages or lose focus, selection, or scroll.
- Unsupported backdrops fall back to an opaque semantic background.
...
- `IThemeContext`: Window-scoped theme state.
- `ThemeVariantChanged`: Theme-change event.
- `Senior.<Area>.<Token>`: Public resource key.
...
IThemeContext is window-scoped; ThemeVariantChanged is ordered on the owning UI Dispatcher.
...
Unknown theme falls back to System; missing tokens emit one non-fatal diagnostic; resource cycles fail build validation.
...
Tokens may be added; existing semantics cannot change, and removal/rename needs v2 plus alias migration.
...
- Dark 实时切换: MUST have deterministic automated evidence.
- System colors: MUST have deterministic automated evidence.
- Missing-token fallback: MUST have deterministic automated evidence.
- Independent window themes: MUST have deterministic automated evidence.
