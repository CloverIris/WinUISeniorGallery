...
Defines player-independent sessions, playback state, commands, timeline, Live DVR, and host requests.
...
- `PlaybackSessionId`: Stable session identity.
- `MediaTime`: Monotonic media clock.
- `SeekPreview`: Uncommitted preview position.
- `HostRequest`: 浮窗请求.
...
- UI never references a concrete player; adapters may connect MediaPlayerElement, C++, or remote engines.
- Snapshots carry monotonic Revision and stale revisions are ignored.
- Seek Preview and Commit are distinct, with one commit on release.
- Live DVR bounds may move while position remains on the same MediaTime.
- Full-screen/CompactOverlay/floating actions publish HostRequest and never create windows.
- P0 Media does not depend on Windowing.
...
Adapters receive/coalesce native events in the background; snapshots and command results publish in order on the session UI Dispatcher.
...
Disposed-session commands return cancelled; unsupported commands return rejected; engine failures expose stable code/correlation without cross-window exceptions.
...
Optional capabilities may be added; command, time unit, revision, or HostRequest semantic changes require v2.
