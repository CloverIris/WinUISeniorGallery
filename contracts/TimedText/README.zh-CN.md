...
定义字幕、歌词、ASR 增量修订、翻译轨道和媒体时钟同步。
- 适用于所有消费本契约的生产控件、体验、Gallery 集成、适配器和测试。
- 本契约不把功能专属公共 API 的所有权转移给契约包。

## 术语

- `TimedTextDocument`: 文档根.
- `TimedTextTrack`: 语言.
- `TimedTextSegment`: 带时间的文本段.
- `TimedTextWord`: 可选逐词时间.
...
- Segment/Word 时间区间为半开 [start,end)。
- Track 声明 language、purpose、direction 和稳定 ID。
- ASR 实体 ID 稳定，修订递增；乱序旧修订忽略。
- 无结束时间 interim 在后继或 final 到达时闭合。
- 翻译是关联 Track/内容，不覆盖原文。
- 缺失 Word 时间降级整段高亮。
...
- `TimedTextDocument`: Document→Track→Segment→Word 不可变模型.
- `TimedTextRevision`: interim.
- `ITimedTextClock`: 读取 MediaTime.
...
Provider 在后台解析并生成不可变修订；TimedTextView 在 UI Dispatcher 应用，使用 Playback Contract 单调时钟。
...
非法时间/重叠按策略隔离并诊断；Provider 失败保留最后 final 内容；无翻译显示原文。
...
可新增元数据/状态；改变层级、区间语义、修订排序或翻译关联需要 v2。
...
- 单行: MUST have deterministic automated evidence.
- ASR interim: MUST have deterministic automated evidence.
- 乱序修订: MUST have deterministic automated evidence.
- 缺失 Word 时间: MUST have deterministic automated evidence.
