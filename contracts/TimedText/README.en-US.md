...
- Contract ID: `contract.timed-text.v1`
- Dependencies: contract.media-playback.v1, contract.localization.v1
...
Defines captions, lyrics, incremental ASR revisions, translation tracks, and media-clock synchronization.
...
- `TimedTextDocument`: Document root.
- `TimedTextTrack`: 用途轨道.
- `TimedTextSegment`: Timed text segment.
- `TimedTextWord`: Optional timed word.
...
- Segment/Word ranges are half-open [start,end).
- Track declares language, purpose, direction, and stable ID.
- ASR entity IDs are stable and revisions increase; stale updates are ignored.
- Open interim segments close when a successor or final update arrives.
- Translation is related track/content and never overwrites source text.
- Missing word timing degrades to segment highlighting.
...
- `TimedTextDocument`: Immutable hierarchy.
- `TimedTextRevision`: final.
- `ITimedTextClock`: Reads MediaTime.
...
Providers parse and create immutable revisions in background; TimedTextView applies them on UI Dispatcher using the Playback monotonic clock.
...
Invalid time/overlap is isolated and diagnosed; provider failure retains last final content; missing translation shows source.
...
Metadata/status may be added; hierarchy, interval, revision-order, or translation-link changes require v2.
...
- 滚动: MUST have deterministic automated evidence.
- final: MUST have deterministic automated evidence.
- undefined: MUST have deterministic automated evidence.
- Provider 失败: MUST have deterministic automated evidence.
