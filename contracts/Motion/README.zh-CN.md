...
定义动画策略、持续时间、缓动、Composition 使用、取消和 Reduced Motion。
- 适用于所有消费本契约的生产控件、体验、Gallery 集成、适配器和测试。
- 本契约不把功能专属公共 API 的所有权转移给契约包。

## 术语

- `MotionProfile`: 完整或减少动画策略.
- `TransitionToken`: 语义持续时间与缓动.
- `InterruptibleMotion`: 可被新状态打断的动画.
- `SettledState`: 打断后逻辑终态.
...
- 普通状态切换优先 VisualState；连续滚动、视差和隐式动画可用 Composition。
- 动画不得阻塞输入、命令完成或 Automation 状态。
- 所有运动必须可取消并收敛到最新逻辑状态。
- Reduced Motion 移除空间位移、视差和 shimmer，保留即时状态或短淡入淡出。
- 单一元素不得闪烁超过每秒三次。
- 页面卸载必须取消 Composition callback 和计时器。
...
- `IMotionPolicy`: 窗口作用域动画偏好.
- `MotionPreferenceChanged`: 偏好变化事件.
- `Senior.Motion.<Token>`: 持续时间与缓动资源.
...
MotionPolicy 按窗口作用域；视觉操作仅在 UI Dispatcher，后台线程只计算参数。
...
动画创建失败立即应用终态；设备能力不足降级到无动画，不影响命令结果。
...
可新增 Token/Profile；改变既有 Token 意义或 Reduced Motion 保证需要 v2。
...
- 完整: MUST have deterministic automated evidence.
- 动画打断: MUST have deterministic automated evidence.
- 卸载清理: MUST have deterministic automated evidence.
- 60Hz 与低能力降级: MUST have deterministic automated evidence.
